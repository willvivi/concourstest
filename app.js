...
function detectCsvSep(headerLine) {
  // Count occurrences of ; and , outside quotes to pick the right separator
...
function bindGreffe() {
  ...
  $("deleteAllParticipants").addEventListener("click", () => {
...
function bindAffectations() {
  $("autoAssign").addEventListener("click", () => {
    ...
    const slotsPerTarget = state.config.rhythm === "AB" ? 2 : 4;
    const capacity = state.config.targets * slotsPerTarget;
...
function autoAssignTargets() {
  ...
  const slots = state.config.rhythm === "AB" ? ["A", "B"] : ["A", "B", "C", "D"];
  ...
  const slotsPerTarget = slots.length;
  const capacity = targets * slotsPerTarget;
  ...
  $("importCsv").addEventListener("click", async () => {
    ...
    for (const r of rows) {
      ...
      const p = {
        ...
        debutant: ["1", "true", "oui", "y", "debutant", "débutant"].includes((r.debutant || "0").trim().toLowerCase()),
...
function printQualifByTarget() {
  ...
  for (let t = 1; t <= targets; t++) {
    ...
    const activeSlots = state.config.rhythm === "AB" ? ["A", "B"] : ["A", "B", "C", "D"];
      ...
      <section class="page">
        ...
        <div class="sheet">
          <div class="head">
            ...
            <div class="box small">
              <div><strong>${state.config.rhythm === "AB" ? "AB" : "AB/CD"}</strong></div>
              <div><strong>Blason</strong> : même • zones selon profil</div>
            </div>
          </div>
          <div class="${state.config.rhythm === 'AB' ? 'sheet2' : 'sheet4'}">${blocks}</div>
        </div>
      </section>
    `);
  }
